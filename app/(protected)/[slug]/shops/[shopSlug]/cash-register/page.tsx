...
import { useCashSessionStore } from "@/lib/services/business/cashSessionStore";
...
export default function CashRegisterPage() {
  ...
  const { currentSession, loading, fetchCurrentSession, openSession, closeSession } = useCashSessionStore();
  ...
  const openingFloat = currentSession ? Number(currentSession.startFloat) : 0;
  
  // Simulated operational sub-metrics (These will bind natively to your SaleService aggregates)
  const cashSales = currentSession ? 1250.00 : 0; 
  const cashIn = currentSession ? 150.00 : 0;
  const cashOut = currentSession ? 100.00 : 0;
  ...
  return (
    <div className="min-h-screen bg-slate-50/60 p-4 lg:p-8 text-slate-900 antialiased font-sans">
      ...
      {/* 🔐 INITIAL WORKSPACE INTERCEPTOR: INITIALIZE TERMINAL SESSION DIALOG */}
      <Dialog open={!currentSession && !loading}>
        <DialogContent className="sm:max-w-[425px] bg-white rounded-2xl p-6" onInteractOutside={(e) => e.preventDefault()}>
          ...
          <Card className="rounded-2xl border border-slate-200/80 bg-white shadow-sm h-full flex flex-col justify-between">
            <CardContent className="p-6 space-y-4">
              ...
              <div className="space-y-3 text-xs font-medium">
                <div className="flex justify-between"><span className="text-slate-400">Register ID</span><span className="font-mono font-bold text-slate-700 truncate max-w-[150px]">{currentSession?.id || "---"}</span></div>
                <div className="flex justify-between"><span className="text-slate-400">Opened By Employee ID</span><span className="font-bold text-slate-800">{currentSession?.openedById || "---"}</span></div>
                <div className="flex justify-between"><span className="text-slate-400">Opened At</span><span className="font-bold text-slate-800">{currentSession ? new Date(currentSession.openedAt).toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'}) : "---"}</span></div>
            ...
            <CardContent className="p-6">
              <div className="flex items-center justify-between mb-4 pb-2 border-b border-slate-100">
                <h3 className="font-bold text-sm text-slate-900 flex items-center gap-1.5">
                  <Store className="w-4 h-4 text-blue-500" /> Core Session Context
                </h3>
              </div>
              <div className="text-xs text-slate-500 space-y-2">
                <p>Ensure that matching shift registers balance perfectly upon completion. Any recorded variances will trigger audit records instantly on database submission operations.</p>
                <div className="p-4 bg-slate-50 rounded-xl border border-slate-100 font-mono text-[11px] text-slate-600">
                  <div>BUSINESS_ID: {currentSession?.businessId || "NULL"}</div>
                  <div>SHOP_ID: {currentSession?.shopId || "NULL"}</div>
                </div>
              </div>
            </CardContent>
          </Card>
                ...
                <table className="w-full text-left border-collapse text-xs font-medium">
                  <thead>
                    <tr className="text-[10px] uppercase font-bold text-slate-400 border-b border-slate-100">
                      ...
                      <th className="pb-2 font-semibold text-right">Float</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-slate-50">
                    {currentSession ? (
                      <tr className="text-slate-700">
                        <td className="py-3 font-mono font-bold text-blue-600 truncate max-w-[120px]">{currentSession.id}</td>
                        <td className="py-3">
                          <Badge className="bg-blue-50 text-blue-700 border-blue-100 text-[10px] shadow-none uppercase">{currentSession.status}</Badge>
                        </td>
                        <td className="py-3 text-right font-bold text-slate-900">GHS {openingFloat.toFixed(2)}</td>
                      </tr>
                    ) : (
                      <tr>
                        <td colSpan={3} className="py-4 text-center text-slate-400 text-xs font-normal">No active running sessions cataloged.</td>
                      </tr>
                    )}
                  </tbody>
                </table>
